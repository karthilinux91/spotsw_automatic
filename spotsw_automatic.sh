...
then
	...
	>$mainpath/tempfiles/store.txt
        if [ flag_os = rhel7 ]	
	then
	iptables -L -n | grep ctstate | grep ACCEPT | grep dpt | awk '{print $7}' | tr -d dpt: >$mainpath/tempfiles/store.txt
	else 
	iptables -L -n  | awk -F "dpt:" '{print $2}'  | grep -o -E '[0-9]+' >$mainpath/tempfiles/store.txt
	fi 
